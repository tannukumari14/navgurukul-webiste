"use client";
import { useState } from 'react';
import "./Header.css";

const Header = () => {
  const [openDropdown, setOpenDropdown] = useState(null); 

  const toggleDropdown = (dropdown) => {
    setOpenDropdown(openDropdown === dropdown ? null : dropdown); 
  };

  return (
    <div className="header">
        <img src="data:image/png;base64,"></img>

      {/* Dropdown for About Us */}
      <div className="about-dropdown">
        <span onClick={() => toggleDropdown("aboutUs")} className="about-dropdown-toggle">
          About Us
        </span>
        <ul className={`about-dropdown-menu ${openDropdown === "aboutUs" ? 'show' : ''}`}>
          <li className="about-dropdown-item">Who we are</li>
          <li className="about-dropdown-item">Team</li><li className="about-dropdown-item">
            Website
            <a style={{ color: "orangered" }} href="https://www.navgurukul.org/" target="_blank" rel="noopener noreferrer">Link</a>
        </li>

          <li className="about-dropdown-item">
            <a style={{ color: "orangered" }} href="https://www.youtube.com/@NavGurukul" target="_blank" rel="noopener noreferrer">Blog</a>
          </li>
        </ul>
      </div>

      {/* Dropdown for Schools */}
      <div className="about-dropdown">
        <span onClick={() => toggleDropdown("schools")} className="about-dropdown-toggle">
          Schools
        </span>
        <ul className={`about-dropdown-menu ${openDropdown === "schools" ? 'show' : ''}`}>
          <li className="about-dropdown-item">School of Programming</li>
          <li className="about-dropdown-item">School of Business</li>
          <li className="about-dropdown-item">School of Design</li>
          <li className="about-dropdown-item">School of Finance</li>
          <li className="about-dropdown-item">BCA</li>
        </ul>
      </div>

      {/* Dropdown for Digital Initiatives */}
      <div className="about-dropdown">
        <span onClick={() => toggleDropdown("digitalInitiatives")} className="about-dropdown-toggle">
          Digital Initiatives
        </span>
        <ul className={`about-dropdown-menu ${openDropdown === "digitalInitiatives" ? 'show' : ''}`}>
          <li className="about-dropdown-item">
            Zuvy
            <a style={{ color: "orangered" }} href="https://zuvy.org/" target="_blank" rel="noopener noreferrer">Visit</a>
          </li>
          <li className="about-dropdown-item">
            Meraki
            <a style={{ color: "orangered" }} href="https://www.merakilearn.org/" target="_blank" rel="noopener noreferrer">Visit</a>
          </li>
          <li className="about-dropdown-item">
            Visit Code India Fellowships
            <a style={{ color: "orangered" }} href="https://www.codeindiafellowship.org/" target="_blank" rel="noopener noreferrer">Visit</a>
          </li>
        </ul>
      </div>

      {/* Dropdown for Get Involved */}
      <div className="about-dropdown">
        <span onClick={() => toggleDropdown("getInvolved")} className="about-dropdown-toggle">
          Get Involved
        </span>
        <ul className={`about-dropdown-menu ${openDropdown === "getInvolved" ? 'show' : ''}`}>
          <li className="about-dropdown-item">Career</li>
          <li className="about-dropdown-item">Partner with Us</li>
        </ul>
      </div>

      <button>Hire For Us</button>
      <button>CSR Inquiries</button>
      <button className="donate" >Donate</button>
    </div>
  );
};

export default Header;
